pipeline {
    ...
    stages {
        stage('Provision VM') {
            steps {
                script {
                    INSTANCE_NAME = "${INSTANCE_PREFIX}-${env.GIT_BRANCH.toLowerCase()}-${env.BUILD_ID}".toLowerCase()
        ...
        stage('Test Services') {
            steps {
                script {
                    ...
                    sshagent(['210402b2-9cf5-4c1b-a3e9-679ff4b02925']) {
                        sh "ssh -o StrictHostKeyChecking=no ${SSH_USER}@${INSTANCE_IP} 'cd test_automation && make test'"
                        sh "scp -o StrictHostKeyChecking=no ${SSH_USER}@${INSTANCE_IP}:test_automation/test_report.txt ."
                    }
                    echo 'DTM Services tested.'
                    sh 'cat test_report.txt'
                }
